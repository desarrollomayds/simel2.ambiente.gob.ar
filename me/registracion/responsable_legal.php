<?
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(!count($errores)){
			if($_POST['accion'] == 'alta'){
				if(empty($_SESSION['DATOS_EMPRESA']['REPRESENTANTES_LEGALES'])){
					$_SESSION['DATOS_EMPRESA']['REPRESENTANTES_LEGALES'] = array();
				}
				...
				$_SESSION['DATOS_EMPRESA']['REPRESENTANTES_LEGALES'][] = $representante;
			}else if($_POST['accion'] == 'baja'){
				$estado = 1;
				foreach($_SESSION['DATOS_EMPRESA']['REPRESENTANTES_LEGALES'] as $indice => $representante){
					if($representante['ID'] == $_POST['id']){
						unset($_SESSION['DATOS_EMPRESA']['REPRESENTANTES_LEGALES'][$indice]);
						$estado = 0;
						break;
					}
				}
		...
		$smarty->display('registracion/responsable_legal.tpl');
	}
