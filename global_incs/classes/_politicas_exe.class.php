		...
		if($pendientes)
		{
			...
			foreach ($pendientes as $row)
			{
				...
				if ($mail_class->enviar($row)) {
					...
					$estado = 'CE';
					echo "<br />";
					var_dump($mail_class->errorInfo);
					echo "<br />";
				}

				$query = "UPDATE emails SET estado = IF('$estado'!='EV' AND cantidad_envios >= '".$this->getParameters('framework::email::cantidad_max_envios')."','CE','$estado'), cantidad_envios='".($row->cantidad_envios+1)."' WHERE id ='".$row->id."'";
