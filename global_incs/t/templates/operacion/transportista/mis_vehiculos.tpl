...
<body>
    ...
    {include file='general/popup.tpl' ID_POPUP='mel4'}
        ...
        <div class="table-responsive">
            ...
            <table class="table table-striped"  id="lista_vehiculos">
                ...
                <tbody>
                    ...
                    <tr {if $VEHICULO->tipo_cambio == 'B'}class="danger"{elseif $VEHICULO->tipo_cambio == 'E'}class="info"{/if}>
                        ...
                        {if empty($VEHICULO->tipo_cambio)}
                            <td class="text-center">
                                <a href="javascript:void(0);" class="btn btn-info btn-xs" onclick="editar_permisos_vehiculo($(this), {$VEHICULO->id})" data-toggle="modal" data-target="#mel_popup" rel='tooltip' data-placement='top' title='Permisos'><i class='fa fa-truck fa-lg'></i></a>
                                <a href="javascript:void(0);" class="btn btn-primary btn-xs" onClick="editar_vehiculo($(this), {$VEHICULO->id})" data-toggle="modal" data-target="#mel_popup"  rel='tooltip' data-placement='top' title='Editar'><i class="fa fa-pencil-square-o fa-lg"></i></a>
                        ...
                        {else}
                            <td class="text-center">A la espera de
                                ...
                                {if $VEHICULO->tipo_cambio == 'E'}
                                    edici&oacute;n 
                                {/if}
                                {if $VEHICULO->tipo_cambio == 'B'}
                                    eliminaci&oacute;n 
                                {/if}
        ...
        <div class="table-responsive">
            ...
            <table class="table table-striped"  id="lista_permisos">
                ...
                <tbody id="contenido_permisos" style="font-size:11px">
                   ...
                   {foreach $VEHICULOS_APROBACION as $VEHICULO}
                    <tr>
                        <td class="text-center" id='dominio'>{$VEHICULO->dominio}</td>
                        <td class="text-center" id='tipo_vehiculo'>{$VEHICULO->tipo_vehiculo}</td>
                        <td class="text-center" id='tipo_caja'>{$VEHICULO->tipo_caja}</td>
