...
<script>
	...
	$('.btn_borrar_permiso_establecimiento').on('click', function() {
		registro_actual = $(this).parent().parent();
		$.ajax({
		   type: "POST",
		   url: "/operacion/transportista/alta_temprana/permiso_establecimiento.php",
	...
	$('.btn_editar_permiso_establecimiento').on('click', function() {
		registro_actual = $(this).parent().parent();
		$.ajax({
		   type: "GET",
		   url: "/operacion/transportista/alta_temprana/permiso_establecimiento.php?establecimiento={/literal}{$ESTABLECIMIENTO.ID}{literal}&id=" + registro_actual.find('#id').html(),
		   dataType: "html",
	...
	$('#btn_agregar_permiso_establecimiento').click(function() {
		...
		$.ajax({
			type: "GET",
			url: "/operacion/transportista/alta_temprana/permiso_establecimiento.php?establecimiento={/literal}{$ESTABLECIMIENTO.ID}{literal}",
			dataType: "html",
			success: function(msg){
				$('#div_popup_2').html(msg);
				$('#div_popup_2').show();
                                 $('#div_popup_2').css("width","600");
				$('#div_popup_2').css("height","250");
