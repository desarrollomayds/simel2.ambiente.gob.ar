<?
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(count($errores) == 0){
			if($_POST['accion'] == 'baja'){
				$conexion = RepresentanteLegal::connection();

				$conexion->transaction();

				try{
					$representante = RepresentanteLegal::first(Array('conditions' => Array('id = ?', $_POST['id'])));
		...
		$informacion = obtener_informacion_representante_legal_drp($_GET['id']);
		$tipos_doc = obtener_tipos_documento();

		$smarty->assign('REPRESENTANTE',   $informacion);
		$smarty->assign('TIPOS_DOCUMENTO', $tipos_doc);
		$smarty->display('drp/operacion/editar_representante_legal.tpl');
	}
