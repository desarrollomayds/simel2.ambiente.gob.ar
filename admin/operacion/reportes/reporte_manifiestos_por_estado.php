...
************************************************************************/
	switch($_POST['estado'] ) {
		...
		case 'AUTORIZADO':
			$filtro_estado = 2;
		break;
		case 'RECIBIDO_OPERADOR':
			$filtro_estado = 3;
		break;
		case 'TRATADO_OPERADOR':
			$filtro_estado = 4;
		break;
		default:
			$filtro_estado = 0;
	...
	WHERE
		#id_protocolo_manifiesto > 0 AND
		(fecha_creacion  >= STR_TO_DATE($_POST[fecha_desde], '%d/%m/%Y') OR '0' = '$flag_fecha_desde' )
		AND (fecha_creacion  <= STR_TO_DATE($_POST[fecha_hasta], '%d/%m/%Y') OR '0' = '$flag_fecha_hasta' )

		AND ( estado = 'p'  OR '1' <> '$filtro_estado' ) # cuando es distinto prende esa condicion y no chequea la anterior
		AND ( estado = 'a'  OR estado = 'e' OR estado = 'f' OR '2' <> '$filtro_estado' ) # cuando es distinto prende esa condicion y no chequea la anterior
		AND ( estado = 'e'  OR '3' <> '$filtro_estado' ) # cuando es distinto prende esa condicion y no chequea la anterior
		AND ( estado = 'f'  OR '4' <> '$filtro_estado' ) # cuando es distinto prende esa condicion y no chequea la anterior
	ORDER BY
		id_protocolo_manifiesto
