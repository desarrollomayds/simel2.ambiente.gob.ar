<?
	...
	$pendientes = obtener_cambios_pendientes('%' . $_POST['criterio'] . '%', $p);
	$cantidad   = obtener_cantidad_cambios_pendientes('%' . $_POST['criterio'] . '%');
	$paginas    = range(0, $cantidad / 20);

	$smarty->assign('USUARIO',    $_SESSION['INFORMACION_USUARIO']);
	$smarty->assign('PAGINAS',    $paginas);
	$smarty->assign('PENDIENTES', $pendientes);
	$smarty->display('drp/operacion/cambios_pendientes.tpl');
